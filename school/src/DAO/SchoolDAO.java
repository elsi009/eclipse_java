...
public class SchoolDAO extends DBconnect{
	...
	public ArrayList<School> searchSelect(String searchWord) {
		...
		try {
			...
			while(rs.next()) {
				School data = new School(rs.getInt("id"),rs.getString("diff"),rs.getString("school_name"),
						rs.getString("address"),rs.getInt("post_code"),rs.getString("tel"),rs.getString("fax"),
						rs.getString("education_office"),rs.getString("public_private"),rs.getString("homepage"));
	...
	public void saveData(String id, int schoolNum) {
		// id 변수는 로그인 아이디 이고, schoolNum은 선택한 학교 번호 (id)이다.
		// insert, delete, update 는 executeUpdate 메서드를 사용하고
		// 결과는 없기에 ResultSet으로 안받아도 된다.
		// 내가 선택한 학교번호가 테이블에 저장될 수 있게 saveData 메서드 내용을 
		// 완성하세요
		...
		try {	
			...
			System.out.println("데이터 베이스 저장 실패");
			e.printStackTrace();
		}
	}
